...
class SelectPictrueActivity :
    ...
    private val gridViewItemClickPath = Constant.PLUS
    ...
    override fun initView() {
        ...
        val cols = UsageUtil.getNumColumn(this)
        binding.myGridView.setNumColumns(cols)
        imagePathsList.add(Constant.PLUS)

        var adapter = BitmapAdapter(imagePathsList, this)
    ...
    private fun toPickPhoto(position: Int, path: String) {
        if (Constant.PLUS.equals(path)) {
            val intent = PickImageIntent(this)
            //设置为多选模式
            ...
            val intent = PreViewImageIntent(this)
        ...
        if (resultCode == Activity.RESULT_OK) {
            when (requestCode) {
                ...
                REQUEST_PREVIEW_CODE -> if (data != null) {
                    ...
                    adapter = BitmapAdapter(imagePathsList, this)
                    binding.myGridView.setAdapter(adapter)
                }
            }
        }
